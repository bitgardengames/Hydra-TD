return {
	...
	moduleDesc = {
		...
		slow_glacier_core = "Each hit applies heavy slow and bonus damage vs slowed enemies.",
		slow_permafrost = "Each hit applies a light slow and creates 1 shard shot.",
		slow_frost_nova = "Each hit applies a steady slow and deals extra damage to slowed enemies.",
		slow_shatterburst = "Shots pierce enemies, gain damage through crowds, and apply slow on hit.",
		slow_cold_snap = "Each hit applies a heavy slow and gains major bonus damage vs slowed enemies.",
		slow_black_ice = "Each hit applies slow and deals splash damage in a wider area.",
		slow_frost_shards = "Each hit applies light slow and spawns 1 shard shot.",
		slow_shatter = "Each hit applies medium slow and bonus damage vs slowed enemies.",
		slow_snowball = "Shots pierce, gain damage per enemy hit, and apply slow on hit.",
		slow_lead_freeze = "Each hit applies heavy slow and high bonus damage vs slowed enemies.",
		slow_wide_chill = "Each hit applies slow and deals splash damage in a wide area.",
		slow_absolute_zero = "Each hit nearly stops enemies, but follow-up damage is lower.",
		slow_hailstorm = "Each hit applies light slow and bursts into shard volleys for crowd damage.",
		lancer_overdrive = "Every 4th hit fires an overdrive round with +140% damage.",
		lancer_volley = "Each hit releases 2 split follow-up shots.",
		lancer_ricochet = "Shots bounce to a nearby enemy after impact.",
		lancer_sustained_barrage = "Fires in 6-shot cycles; shots 1-3 deal bonus damage.",
		lancer_opening_strike = "Hits deal high bonus damage to enemies above 80% HP.",
		lancer_rail_lance = "Shots switch to linear movement and gain pierce.",
		lancer_focus_fire = "Repeated hits on the same target build bonus damage stacks.",
		poison_blight = "Applies strong poison with a lower max stack count.",
		poison_plague = "Applies weaker poison per stack, but allows more max stacks.",
		...
		poison_cull_weak = "Hits deal bonus direct damage based on target poison stacks.",
		poison_corrupt_strong = "Each hit splashes poison to up to 2 nearby enemies.",
		poison_hemotoxin = "Poison damage scales with target missing HP.",
		poison_pandemic = "On poisoned enemy death, poison spreads to nearby enemies and can chain again.",
		cannon_siege_shells = "Fires fewer, heavier shells with larger blasts and higher impact damage.",
		cannon_rapid_mortar = "Fires faster, lighter shells for higher sustained splash damage.",
		cannon_cluster_payload = "Shells split into 3 smaller bomblets on impact.",
		cannon_shockwave = "Impacts launch enemies, then emit a damaging shockwave.",
		...
		shock_storm_coil = "Chain lightning jumps to many nearby enemies.",
		shock_overcharge = "Chain lightning makes fewer jumps but keeps more damage per jump.",
		shock_forked_arc = "Chain lightning jumps between enemies and forks into side arcs.",
		shock_static_surge = "Repeated hits on the same target build stacking surge damage.",
		shock_crowd_search = "Adds extra chain jumps and stronger side forks for higher crowd damage.",
		shock_boss_focus = "Boosts repeated-hit surge damage and adds small endpoint bursts.",
		shock_thunderstorm = "Greatly extends chain reach and adds side arcs for max crowd damage.",
		shock_meltdown = "Chain endpoints detonate for extra area damage.",
		plasma_focused_core = "Fires a focused plasma bolt that burns enemies in a tight area.",
		plasma_unstable_core = "Fires an unstable plasma bolt that burns a wider area over time.",
