...
# Rising envelopes, expressed as required/affordable DPS basis points. They are
# deliberately wide: this is a regression alarm, not a prescriptive solution.
RATIO_BANDS = [(250, 30_000), (500, 35_000), (750, 40_000), (1_000, 45_000),
               (1_250, 50_000), (1_500, 55_000), (1_750, 60_000), (2_000, 65_000),
               (2_250, 70_000), (2_500, 90_000)]
...
def build_report() -> dict:
    ...
    report = {"format_version": 1, "units": {"durability": "threat points",
    ...
    for diff_name, diff in diffs.items():
        ...
        for map_index, (map_id, waves) in enumerate(maps.items(), 1):
            ...
            for wave_no, groups in enumerate(waves, 1):
                ...
                rows.append({"wave": wave_no, "effective_durability": durability,
...
def write_docs(report: dict) -> None:
    ...
    for difficulty, maps in report["difficulties"].items():
        ...
        for map_id, waves in maps.items():
            lines += [f"### {map_id}", "", "| Wave | Threat | Peak 5s | Income | Pre-wave $ | Affordable DPS | Specialists | Req. DPS | Ratio (bp) |",
                      "|---:|---:|---:|---:|---:|---:|:---|---:|---:|"]
            for r in waves:
                lines.append(f"| {r['wave']} | {r['effective_durability']} | {r['peak_five_second_durability']} | {r['full_clear_kill_income']} | {r['purchasing_power_before_wave']} | {r['affordable_sustained_damage']} | {', '.join(r['specialist_coverage']) or 'none'} | {r['required_damage']} | {r['required_to_affordable_bp']} |")
            lines.append("")
